...
public class GameLogic {
    ...
    public static  SudokuGame getNewGame(){
        return new SudokuGame(
                GameState.NEW,
                GameGenerator.getNewFameGrid()
        );
    }

    public static GameState checkForCompletion(int[][] newGridState){
        if(sudokuIsInvalid(newGridState)) return GameState.ACTIVE;
        if(tileAreNotFilled(newGridState)) return GameState.ACTIVE;
    ...
    private static boolean tileAreNotFilled(int[][] grid){
        for(int xIndex = 0; xIndex < GRID_BOUNDARY; xIndex++){
            for(int yIndex = 0; yIndex < GRID_BOUNDARY; yIndex++){
